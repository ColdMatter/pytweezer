...
class DataSummary:
    ...
    def initDataq(self):
        new_streamlist = self.props.get("datastreams", []) + [
            "Experiment.start",
            "Experiment.end",
        ]
    ...
    def processIncomingData(self, msg, newdata, A):
        ...
        if msg == "Experiment.start":
            ...
            self.task = max(self.task, newdata["_task"])
            # self.newTask()
            ...
            len(self.incompleteData) > 0
        ):  # process data only after experiment start has been detected
            if msg == "Experiment.end":
                ...
                for line in reversed(self.incompleteData):
                    ...
                    if (
                        line["_starttime"] < newdata["timestamp"]
                    ):  # take last run starting before image was recorded
                        dic = {
                            msg + "." + k: v for k, v in newdata.items() if k[0] != "_"
                        }  # don't take parameters starting with _,prepend channelname to key
                        line.update(dic)
                    ...
                    print(
                        "datamgr.py Warning data discarded because: data arrived after timeout or more than once per run",
                        msg,
                    )  # ,'incomp', repr(incomp))
        # print()
    ...
    def _processCompleteData(self):
        ...
        # print(f'incomp _pocessCompleteData: {self.incompleteData}') # this is empty at first when the ablation mirror script sends data but after a couple of ablation mirror executions it one for a sudden containes something
        if len(self.incompleteData) == 0:
            return
        # print('dsdad',set(self.streamlist),set(incomp[0]['_streams']))
        while len(self.incompleteData) > 0 and set(self.streamlist) == set(
            self.incompleteData[0]["_streams"]
        ):
        ...
        while (
            len(self.incompleteData) > 0
            and (
                "_endtime" in self.incompleteData[0]
                and time.time()
                > self.incompleteData[0]["_endtime"] + self.props.get("timeout", 1)
            )
            or self.lastupdate + self.props.get("timeout_without_endtime", 60)
            < time.time()
        ):
    ...
    def savetoFile(self, filename, folder="folder"):
        ...
        try:
            ...
            streams_columns = [
                col for col in data_pandas.columns if col.startswith("_streams")
            ]
            data_pandas.drop(columns=streams_columns, inplace=True)

            dtype_columns = [
                col for col in data_pandas.columns if col.endswith(".dtype")
            ]
            data_pandas.drop(columns=dtype_columns, inplace=True)

            shape_columns = [
                col for col in data_pandas.columns if col.endswith(".shape")
            ]
            ...
            dt_list = [
                dt if dt != "|O" else "S400"
                for dt in np.array(data_pandas.dtype.descr)[:, 1]
            ]
            dt_list_names = [
                (name, dt) for name, dt in zip(data_pandas.dtype.names, dt_list)
            ]
            ...
            with h5py.File(filename, "a") as f:
                f.create_dataset(
                    folder + "/" + self.name + "%i" % time.time(), data=data_pandas
                )
            ...
            with h5py.File(filename.replace(".h5", "_old_routine.h5"), "a") as file:
                ...
                try:
                    ...
                    for enum, values in enumerate(data):
                            ...
                            try:
                                if "_streams" not in k:
                                    ...
                                    if type(v) is list:
                                        if (
                                            k != "_expName"
                                        ):  # BaLi's little helper wrongly emits lists
                                            for i, v_i in enumerate(v):
                                                if (
                                                    f"{k}{i}"
                                                    not in np.array(flist)[:, 0]
                                                ):
                                                    logger.warning(
                                                ...
                                                if type(v_i) == str:
                                                    dataset[f"{k}{i}", index] = (
                                                        v_i.encode("ascii", "ignore")
                                                    )
                                                else:
                                                    dataset[f"{k}{i}", index] = v_i
                                    elif type(v) == str:
                                ...
                                logger.error(
                                    "savetoFile(): Cannot store k=%s, v=%s; %s.",
                                    k,
                                    v,
                                    e,
                                )
                        index += 1
                        if (
                            progress + 0.1 < (enum + 1) / len(data)
                            or (datetime.datetime.now() - last_output).seconds / 60.0
                            > 5
                        ):
                            # Every 10% progress or 5 min:
                            progress = (enum + 1) / len(data)
                            last_output = datetime.datetime.now()
                            end = start + (datetime.datetime.now() - start) / (
                                enum + 1
                            ) * len(data)
                            logger.info(
                                "savetoFile(): %s %%,\tshould be done at %s.",
                                np.round(100 * progress, 1),
                                end,
                            )
                    return True
                except Exception as e:
                    logger.error(
                        "savetoFile(): Error while saving into h5 file,\n%s", e
                    )
                    return False
    ...
    def _determineFormatstring(self, data):
        ...
        for run in data:
            for k, v in run.items():
                if "_streams" not in k and type(v) is list and k in keylist:
                    try:
                        v.extend(keylist[k])
                        run[k] = list(set(v))
                    except Exception as e:
                        ...
                        logger.debug(
                            "list(set(keylist[k] + v)) = %s",
                            list(set(keylist[k] + v)),
                        )
            keylist.update(run)
        # print('\n\nkeylist:', keylist)
    ...
    def get_unique_flist(self, flist):
        ...
        for col in fset[counts > 1]:
            types[col] = [elt[1] for elt in flist if elt[0] == col]
    ...
    def savetoFile(self, filename, folder="folder"):
        ...
        try:
            with h5py.File(filename, "a") as file:
                file.create_dataset(
                    folder + "/" + self.name + "dat%i" % time.time(), data=dat
                )
            success = super().savetoFile(filename, folder)
            return success
        except Exception as e:
            logger.error(
                "SingleDataSummary - savetoFile(): Error while saving into h5 file,\n%s",
                e,
            )
            return False
        # self.clear()
    ...
    def savetoFile(self, filename, folder):
        with h5py.File(filename, "a") as file:
            ...
            for i, a in enumerate(self.arrays):
                # save images
                group.create_dataset(f"{i:04d}", data=a)

                # save x and yaxis
                group.create_dataset(f"{i:04d}x", data=self.axis[i][0])
                group.create_dataset(f"{i:04d}y", data=self.axis[i][1])
