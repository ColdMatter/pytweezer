...
def sort_into_grid(centers, grid_shape=(2, 2)):
    ...
    if len(centers) != num_row * num_col:
        ...
        y_sorted_array = sorted_centers.reshape((num_row, num_col, 2))
        sorted_array = np.array(
            [np.array(sorted(row, key=lambda p: p[1])) for row in y_sorted_array]
        )
        grid_positions = {}
...
def visualize_results(
    ...
    cropped_bin_image = (cropped_image - image_array.min()) / (
        image_array.max() - image_array.min()
    )
    threshold_bin = (threshold - image_array.min()) / (
        image_array.max() - image_array.min()
    )
    ...
    img_bin_sigmoid = sigmoid(
        cropped_bin_image, bin_sharpness, bin_thresh_factor * threshold_bin
    )
...
def detect_loading_threshold(counts):
    ...
    c = (
        mu_bg**2 / (2 * var_bg)
        - mu_sig**2 / (2 * var_sig)
        - np.log(np.sqrt(var_sig / var_bg))
    )

    # The quadratic formula yields two roots; we want the one between the means
    roots = np.roots([a, b, c])
    threshold = next(r for r in roots if mu_bg < r < mu_sig)
