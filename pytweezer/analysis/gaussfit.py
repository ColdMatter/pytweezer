''' Perform a 1D gaussian fit on a datataset

Fitfunction:
    :math:`f(x) = \\frac{A_0}{\sqrt{2\pi}\sigma} e^{\\frac{-(x-\\mu)^2}{2\\sigma^2}} + C`
...
import pyximport
pyximport.install()
from pytweezer.analysis.src.gaussfit import gaussian_fit
...
class Gaussfit:
    ...
    def __init__(self,name):
        ...
        print('gaussfit.py subscriptions: ',self._datastreams)
    ...
    def run(self):
        while True:
            ...
            if msg!= None:
                ...
                try:
                    x, y_fit, converged, parameters = gaussian_fit(x, y, verbose=self._verbose,
                ...
                head['converged']=converged
                head['height']=parameters[3]/(parameters[1]*np.sqrt(2*np.pi))
                head['peakdensity_3D']=parameters[3]/(parameters[1]*np.sqrt(2*np.pi))**3
                # send also 1/e^2 value i.e. waist radius
                head['waist'] = 2*parameters[1]
