...
class ScrollPlot(pg.PlotItem):
    ...
    def __init__(self, name, parent, **kargs):
        ...
        lc = self.props.get(
            "linecolors", [[55, 126, 184], [166, 206, 227], [255, 127, 0]]
        )
        ...
        self.running_average_window_size = self.props.get(
    ...
    def feedData(self, data_chunk, data_slice=[0, 1], count=True):
        ...
        if self.running_average:
            self.running_average_array = self.running_mean(
                data_array[0], self.running_average_window_size
            )
    ...
    def updatePlot(self):
        ...
        self.lplot.setData(
            x=self.x[: self.feed_count], y=self.data_array[0, indx_feed_count:]
        )
        if self.running_average:
        ...
        if self.datastream.has_new_data():
            ...
            try:
                ...
                if self.dataname == "":
                    ...
                    print(e)
                    print(
                        "scrollwindow.py selected data not found in dataset: ",
                        self.dataname,
                        di,
                    )

    def setScatterplot(self):
        self.lplot = self.plot(
            pen=None,
            name=self.dataname,
            symbol="o",
        )
...
class ScrollWindow(BMainWindow):
    ...
    def addPlot(self):
        p = ScrollPlot(f"Plot {self.plotcount}", parent=self)
        self.win.addItem(p)
