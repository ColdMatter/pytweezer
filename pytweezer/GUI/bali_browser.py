...
from pytweezer.GUI.experiment_manager import ExperimentManager
from pytweezer.GUI.prepstation import PrepStation
from pytweezer.GUI.looper import Looper
from pytweezer.GUI.experiment_queue import ExperimentQ
...
class BaliBrowser(QMainWindow):
    ...
    def __init__(self):
        ...
        self.paramDir = balipath + '/configuration/bali_browser/experiment_params'  # path for storing parameters
        ...
        self.prepStation = PrepStation(browser=self, parent=self)
    ...
    def closeEvent(self, event):
        ...
        # /home/bali/scripts/pytweezer/configuration/bali_browser/loopfile/loopfile.json
        # /home/bali/scripts/pytweezer/configuration/bali_browser/prepfile/prepfile.json

        path = '/home/bali/scripts/pytweezer/configuration/bali_browser/'
        for fname in ['loopfile/loopfile.json', 'prepfile/prepfile.json']:
...
class ExperimentWindow(QWidget):
    ...
    def __init__(self, filepath, props, parent=None, browser=None):
        ...
        try:
            ...
            for member in members:
                ...
                if inspect.getmro(obj)[1].__name__ == 'BaliExperiment':
                    experiment = obj()
                    experiment.build()
    ...
    def init_ui(self):
        ...
        submitButton.setIcon(QtGui.QIcon(balipath + '/pytweezer/icons/run.svg'))
        ...
        for i, argument in enumerate(self.experiment._arguments):
            if type(argument) == pytweezer.experiment.baliexperiment.NumberValue:
                ...
                print_error('bali_browser error argument type not known', 'error')
                box = None
            if box:
    ...
    def reset(self):
        ...
        if not os.path.isfile(filepath):
            print_error('bali_browser.py - reset(): Save defaults before restoring defaults!', 'warning')
            return
    ...
    def load_params(self, filepath=None):
        ...
        for box in self.argument_boxes:
            if box.parName in loaded_params:
                ...
                #print('load_params val', val)
            else:
                val = box.value
                print_error('bali_browser.py - load_params(): Param {0} not in save file, setting to default value {1}.'.format(box.parName, box.value), 'warning')
    ...
    def submit_to_queue(self):
        ...
        task, taskDict = self.setup_task_dict()
        taskDict['experiment'] = self.experiment
        print_error('\nbali_browser.py - submit_to_queue(): Submitted task dict:\n{0}\n'.format(taskDict), 'weak')
        self.queue.tableModel[task] = taskDict
        self.check_defaults(None)
    ...
    def submit_to_prepper(self):
        ...
        self.setup_scan()
        task, taskDict = self.setup_task_dict()
        print_error('\nbali_browser.py - submit_to_prepper(): Submitted task dict:\n{0}\n'.format(taskDict), 'weak')
        self.prepStation.prepList.append(taskDict)
    ...
    def setup_scan(self):
        ...
        if self.scan != '--NONE--':
            self.scanSettings = self.experiment._props.get('scans/' + self.scan, {})

            # generate numpy array containing scan values for example
            # np.array([[0,0],[1,10],[2,20]]) for two parameters scanned
            if 'listgenerators' in self.scanSettings:
                generators = self.scanSettings.get('listgenerators', [''])
                scanValues = []
                self.scanPars = self.scanSettings.get('parameters', [])
                for i, gen in enumerate(generators):
                    # print(self.scanPars[i])
                    if self.scanPars[i] != '--NONE--':
                        scanValues.append(eval(gen))
                    else:
                        scanValues.append(np.zeros_like(eval(generators[0])))
                scanValues = np.vstack(scanValues).T
            else:
                scanValues = eval(self.scanSettings.get('listgenerator', ''))
                if scanValues.ndim == 1:
                    scanValues = scanValues[:, np.newaxis]
            send_info('bali Experiment scanning: {} {}'.format(self.scanPars, scanValues))

            self.nRuns = scanValues.shape[0]
            self.scanValues = scanValues.tolist()
            self.scanValues = round_floating_prec(self.scanValues)  # TODO: use n_digits
            self.scanSequence = np.arange(self.nRuns)
            self.scanSequence = self.scanSequence.tolist()
            if self.scanSettings.get('Randomize', True):
                np.random.shuffle(self.scanSequence)
        else:
            self.nRuns = 1
            self.scanPars = []
            self.scanValues = []
            self.scanSequence = []
        print_error('bali_browser.py - setup_scan: Scan values are {0}'.format(self.scanValues), 'weak')
    ...
    def setup_task_dict(self):
        ...
        taskDict['label'] = lastSet
        if self.taskLabel.text():
            taskDict['label'] = self.taskLabel.text() + ' ' + taskDict['label']
        if self.scanCombo.currentText() != '--NONE--':
            taskDict['label'] = 'scan: {} {}'.format(self.scanCombo.currentText(), taskDict['label'])
        taskDict['expName'] = self.name
        taskDict['run'] = 0
        taskDict['priority'] = self.prioQSB.value()
        taskDict['args'] = self.build_argument_dict()
        taskDict['nRuns'] = self.nRuns
        taskDict['scanpars'] = self.scanPars
        taskDict['scanvals'] = self.scanValues
        taskDict['nReps'] = self.nRepsQSB.value()
        taskDict['scansequence'] = self.scanSequence
        taskDict['repetition'] = 0
        taskDict['terminated'] = False
        taskDict['filepath'] = self.filepath
        if self.scheduleBox.isChecked():
            taskDict['status'] = 'Waiting'
            taskDict['dueDateTime'] = self.dateTimeEdit.dateTime().toString()
        else:
            taskDict['status'] = 'Queued'
            taskDict['dueDateTime'] = QDateTime.currentDateTime().toString()
        return task, taskDict
    ...
    def build_argument_dict(self):
        ...
        for arg in self.argument_boxes:
            ...
            if argDict[arg.name] == 0 and arg.value != 0:
                print_error('bali_browser.py - build_argument_dict(): Error while rounding!', 'error')
    ...
    def argument_dicts(self):
        ...
        return [arg.__dict__.copy() for arg in self.experiment._arguments]
...
class SequenceEditor(QDialog):
    ...
    def __init__(self, experimentWindow, parent=None):
        super().__init__(parent)
        self.expWindow = experimentWindow
        self.setWindowTitle("Sequence editor")
        self.layout = QVBoxLayout()
        self.layout.addWidget(QLabel('Sequence Name:'))
        self.nameEdit = QLineEdit(self.expWindow.scanCombo.currentText())
        self.layout.addWidget(self.nameEdit)

        self.number_scans = 3
        snake_scan = False
        if self.nameEdit.text() != '--NONE--':
            defaults = self.expWindow.experiment._props.get('scans/' + self.nameEdit.text(), {})
    ...
    def init_scans(self):
        ...
        for i in range(len(self.parameterCombos), self.number_scans):
            ...
            paramNames = [box.parName for box in self.expWindow.argument_boxes]
        ...
        self.layout.addLayout(self.glayout)  # TODO: this is needed but somehow a reduced version of it
        self.setLayout(self.layout)
    ...
    def update_parameter(self, parname, n):
        ...
        if parname == '--NONE--':
            return
        for arg in self.expWindow.argument_dicts():
            if arg['name'] == parname:
                argument = arg
                break
        else:
            send_error('bali_browser coding error')
            return
        ...
        for box in [self.startSpins[n], self.stopSpins[i]]:
            print_error('bali_browser.py - update_parameter(): Argument dict for {0}: {1}'.format(parname, argument), 'weak')
            ...
            if 'step' in argument:
                ...
                box.setSingleStep(1)
                print_error('bali_browser.py - update_parameter(): Set step=1 for box {0}.'.format(parname), 'weak')
            ...
            if 'ndecimals' in argument:
                ...
                box.setDecimals(0)
                print_error('bali_browser.py - update_parameter(): Set decimals=0 for box {0}.'.format(parname), 'weak')
            ...
            if 'minval' in argument and 'maxval' in argument:
                ...
                box.setRange(0, 1)
                print_error('bali_browser.py - update_parameter(): Set range=(0, 1) for box {0}.'.format(parname), 'weak')
            ...
            if 'display_multiplier' in argument:
                ...
                box.multiplier = 1
                print_error('bali_browser.py - update_parameter(): Set multiplier=1 for box {0}.'.format(parname), 'weak')
    ...
    def update_values(self):
        ...
        if self.nameEdit.text() != '--NONE--':
            ...
            self.expWindow.experiment._props.set(
...
class BaliFileSelector(QWidget):
    ...
    def __init__(self, browser, parent=None):
        ...
        tree.setRootIndex(model.index(self.props.get('experiments_dir', balipath + '/experiments')))
...
def main():
    ...
    icon = QtGui.QIcon()
    icon.addFile('../pytweezer/icons/pytweezer_experiment_browser_icon.svg')
...
def exception_hook(exctype, value, traceback):
    print_error('bali_browser.py: error {0} {1}'.format(exctype, value), 'error')
    print(traceback)
