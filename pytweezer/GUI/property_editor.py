...
from pytweezer.servers import Properties, send_error, tweezerpath
...
class TreeEdit:
    ...
    def __init__(self, name, parent, props=None, subtree="/"):
        ...
        proptree = (
            self.props.get("/")
            if self.subtree == "/"
            else self.props.get(self.subtree[:-1])
        )

        # create pyqtGraph Parametergroup
        self.pGroup = Parameter.create(
            name="params", type="group", children=self._parsedict(proptree)
        )
                    ...
                    children.append(
                        {
                            ...
                            "type": "list",
                            "value": value,
                            "values": [value] + self.props.get("/Servers/roilist", []),
                        }
                    )
                ...
                children.append(
                    {
                        ...
                        "value": value,
                        "step": 10
                        ** floor(log10(abs(value / 1000 if value != 0 else 1))),
                        "decimals": 10,
                    }
                )
                    ...
                    children.append(
                        {
                            "name": key,
                            "type": "group",
                            "children": self._parsedict(value),
                        }
                    )
        return children
    ...
    def change(self, params, changes):
        ...
        for param, change, data in changes:
            ...
            try:
                ...
                logger.error(f"property_editor change error {e!s}")
    ...
    def _update(self, path):
        ...
        subtree = self._parsedict(
            {foundpath.split("/")[-1]: self.props.get(self.subtree + foundpath[1:])}
        )[0]
    ...
    def update(self):
        ...
        for change in changes:
            if change == self.subtree:
                ...
                len(change) > len(self.subtree)
                and change[: len(self.subtree)] == self.subtree
            ):
...
class PropEdit(BWidget):
    ...
    def save(self):
        ...
        fname = QFileDialog.getSaveFileName(
            self,
            "Save into file",
            tweezerpath + "/configuration/user",
