...
class ExperimentManager:
    ...
    def run(self, taskNr, progress_callback=None):
        ...
        if taskNr not in self.expDict.keys():  # in case the task is deleted before it can be run
            ...
            return
        taskDict = self.expDict[taskNr]
        self.taskNr = taskNr
        self.name = taskDict['expName'] + ' ' + taskDict['label']
        self.experiment = taskDict['experiment']
        self.argDict = taskDict['args']
        self.nRuns = taskDict['nRuns']
        self.scanPars = taskDict['scanpars']
        self.scanValues = np.array(taskDict['scanvals'])
        self.nReps = taskDict['nReps']
        self.scanSequence = taskDict['scansequence']
        self.terminated = taskDict['terminated']
        self.rep = taskDict['repetition']
        self.runNr = taskDict['run']
        self.experiment.expName = taskDict['expName']
        self.browser.props.set('ExperimentQ/current_scan', self.scanPars)

        self.start_measurement()  # pass argdict to the experiment
        """Repeat the experiment nRep times."""
        while self.rep <= self.nReps and self.running:
    ...
    def _runloop(self):
        ...
        if self.taskNr not in self.expDict.keys():  # in case the task is deleted while running
            ...
            return
        while self.runNr < self.nRuns and self.running:
            ...
            for i, parname in enumerate(self.scanPars):  # modify value(s) for scan
                if parname != '--NONE--':
                    self.set_dict(parname, self.scanValues[self.scanSequence[self.runNr], i])

            self.set_run_nr(self.runNr)
            # time.sleep(0.5)
    ...
    @staticmethod
    def due_check(taskDict):
        if 'dueDateTime' not in taskDict.keys():
            return True
        else:
            currentDateTime = QDateTime.currentDateTime()
            dueDateTime = QDateTime.fromString(taskDict['dueDateTime'])
    ...
    def start_measurement(self):
        ...
        for arg in self.argDict.keys():
            self.set_dict(arg, self.argDict[arg])
    ...
    def start_sequence(self):
        """Call the experiment start_sequence function."""
        self.experiment.start_sequence()
    ...
    def end_measurement(self):
        ...
        self.experiment.end_measurement()
