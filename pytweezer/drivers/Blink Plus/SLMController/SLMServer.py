...
class SLMHardwareController:
    def __init__(self):
        ...
        if num_boards_found.value == 1:
            ...
            print(
                f"width = {width}, height = {height}, depth = {depth} bits, serial number = {serial}"
            )
            ...
            if width == 1024:
                load_lut_status = self.slm_lib.Load_LUT_file(
                    self.board_number,
                    b"C:\\Program Files\\Meadowlark Optics\\Blink Plus\\LUT Files\\slm40_at852.LUT",
                )
            ...
            retVal = self.slm_lib.Write_image(
                self.board_number,
                BlankImage.ctypes.data_as(POINTER(c_ubyte)),
                self.timeout_ms,
            )
            if retVal != 1:
                print("DMA Failed")
    ...
    def update_mask(self, mask_array):
        ...
        print("[Hardware] Phasemask updated.")
    ...
    def run_sequence(self, mask_sequence, fps):
        for i in range(mask_sequence.shape[0]):
            ...
            time.sleep(1 / fps)
            print(
                f"[Hardware] Displaying frame {i + 1}/{mask_sequence.shape[0]} at {fps} FPS."
            )

        print("[Hardware] Phasemask sequence uploaded.")
        ...
        while True:
            ...
            try:
                if command == "UPDATE_MASK":
                    ...
                    socket.send_json(
                        {"status": "error", "msg": f"Unknown command: {command}"}
                    )
